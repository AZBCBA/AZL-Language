...
- **P0.1c (release):** **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** step **4** — harness + same interpreter file; **two** **`emit interpret`** (same **`code`**) for **cache hit** tokenize + parse, plus **third** (**two-line** **`say`**, **`AZL_SPINE_DEPTH_*`**), **fourth** (**three-line** **`say`**, **`AZL_SPINE_TRIPLE_*`**), and **fifth** (**four-line** **`say`**, **`Q5a`**–**`Q5d`** — compact for **`::ast.nodes`** **255**-byte cap) **`emit interpret`**; **interpret→tokenize→parse→execute→execute_complete** (**`docs/ERROR_SYSTEM.md`** **548–560**); still not a claim of full structured **`::ast.nodes`** / every in-file **`::execute_*`** path.
