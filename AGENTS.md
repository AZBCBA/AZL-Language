...
- **P0.1c (release):** **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** step **4** — harness + same interpreter file; **two** **`emit interpret`** (same **`code`**, **`say x`**) for **cache hit** tokenize + parse, plus **third** (**two-line** **`say`**, **`AZL_SPINE_DEPTH_*`**), **fourth** (**three-line** **`say`**, **`AZL_SPINE_TRIPLE_*`**), **fifth** (**four-line** **`say`**, **`Q5a`**–**`Q5d`** — compact for **`::ast.nodes`** **255**-byte cap), **sixth** and **seventh** (duplicate **`say 'AZL_S6_ONLY'`** — **≥4** **`(cache hit)`** lines total with **`say x`** pair), **eighth** (**`say 'AZL_S8_MARK'`**); **interpret→tokenize→parse→execute→execute_complete** (**`docs/ERROR_SYSTEM.md`** **548–562**); still not a claim of full structured **`::ast.nodes`** / every in-file **`::execute_*`** path.
