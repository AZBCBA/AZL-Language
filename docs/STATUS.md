...
- **P0 spine (remainder):** widen **`tools/azl_semantic_engine/`** until it can execute **`azl/runtime/interpreter/azl_interpreter.azl`** as AZL source (or equivalent); today semantic spine = Python **C-minimal-parity** subset — see [PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md) **Layer 1**.
