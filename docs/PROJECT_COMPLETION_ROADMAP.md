...
**Principle:** Prefer increments that unlock a **contiguous** region of **`azl/runtime/interpreter/azl_interpreter.azl`** on the **semantic spine** hosted by **`tools/azl_semantic_engine/minimal_runtime.py`** (**current:** transitional **Python** stepping surface on the **opt-in** launcher; **target:** **C orchestrates; AZL interprets** with that **`.azl`** file as the semantic spec). **Gate G2** pins **spec anchor** vs **spine exec carrier** in the `--semantic-owner` probe only — it does **not** change the fact that the **default** enterprise path is still **C minimal** ([RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md) **Gate G2**). Extend **C minimal** only where **`scripts/check_azl_native_gates.sh`** already enforces parity. **Gates F2–F167** (and successors) are **regression anchors**; they do **not** replace **deeper interpreter smokes** that prove the **real file** advances.

**Semantic / runtime gaps vs milestones** (living matrix, narrow claims until **Phase F**): [TIER_B_BACKLOG.md](TIER_B_BACKLOG.md) § **P0.1 — Semantic / runtime gaps (explicit, narrow claims)**.

| Phase | Intent | Exit criteria (claim “done” only if these hold) | Primary verification |
|-------|--------|---------------------------------------------------|----------------------|
| **A — Parity safety net** | C ↔ Python stay aligned on the **contracted** subset | `bash scripts/check_azl_native_gates.sh` exits **0** (includes **F**, **F2–F167**, **G**, **G2**, **H**, engine build as wired) | Native gates |
| **B — Real interpreter file, shallow** | Stub + real source **`init`** completes on spine; behavior-entry harness runs **interpret** pipeline | `bash scripts/verify_azl_interpreter_semantic_spine_smoke.sh` exits **0**; marker **`azl-interpreter-semantic-spine-smoke-ok`** (**ERROR_SYSTEM** **286–290**). `bash scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` exits **0**; marker **`azl-interpreter-semantic-spine-behavior-smoke-ok`** (**ERROR_SYSTEM** **548–562**, **611**) | **`make verify`** steps **3–4** (`run_full_repo_verification.sh`) |
...
- `AZL_RUNTIME_SPINE=azl_interpreter` or `semantic`: `scripts/azl_azl_interpreter_runtime.sh` → `tools/azl_runtime_spine_host.py` → **`tools/azl_semantic_engine/`** (`minimal_runtime.py`), a **Python** executor with **execution parity** to the C minimal contract (say / set / emit / link / component init+behavior / quoted `listen for`) — **bootstrap / parity** carrier on this **non-default** launcher, not the steady-state language spec (**target** remains **`azl_interpreter.azl`** under **C orchestrates; AZL interprets**). **Gate G2** reports **`AZL_SEMANTIC_SPEC_OWNER`** = **`azl_interpreter.azl`** and **`AZL_SPINE_EXEC_OWNER`** = **`minimal_runtime_python`** for the semantic launcher — **without** asserting default spine or full semantic depth. **Gate F2** in `check_azl_native_gates.sh` asserts **byte-identical stdout** vs C on `azl/tests/c_minimal_link_ping.azl`.
...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, **two** **`emit interpret`** (same **`say x`**) + **`(cache hit)`** on the second pass, plus **third** (**two-line** **`say`**, **`AZL_SPINE_DEPTH_*`**), **fourth** (**three-line** **`say`**, **`AZL_SPINE_TRIPLE_*`**), **fifth** (**four-line** **`say`**, compact **`Q5a`**–**`Q5d`** for **`::ast.nodes`** budget), **sixth** and **seventh** (duplicate **`say 'AZL_S6_ONLY'`** — **second** tok/parse cache pair; verify **≥4** **`(cache hit)`** lines), **eighth** (**`say 'AZL_S8_MARK'`**), **ninth** (**`set ::…`** + **`say`** → **`AZL_SPINE_P9_SET_LINE`**) (**`docs/ERROR_SYSTEM.md`** **548–562**, **611**). Does **not** claim full structured **`::ast.nodes`** / every in-file **`::execute_*`** path yet.
