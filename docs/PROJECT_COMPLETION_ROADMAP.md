...
| **E — Vertical slice: execute** | **`listen for "execute"`** — **`::ast` / `::scope`** from payload, **`emit execute_complete`**, **`AZL_USE_VM`** via **`::internal.env`** | **F86**–**F87** landed (**Layer 1**); VM path / **`::execute_ast`** when minimal supports them | Parity gates + smoke when extended |
...
- **Open next (documented):** **`listen for "halt_execution"`**, pre-exec **`::ast.nodes`** loop, VM path when builtins exist — **F88+**; **[RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md)** “Open next”.
