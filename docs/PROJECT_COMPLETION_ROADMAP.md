...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, **two** **`emit interpret`** (same code) + in-file **tok_cache**/**ast_cache** **`(cache hit)`** on the second pass, plus **third** (**two-line** **`say`**, **`AZL_SPINE_DEPTH_*`**), **fourth** (**three-line** **`say`**, **`AZL_SPINE_TRIPLE_*`**), **fifth** (**four-line** **`say`**, compact **`Q5a`**–**`Q5d`** markers for **`Var.v`** **255**-byte **`::ast.nodes`**), and **sixth** (single-line literal **`say`**, **`AZL_S6_ONLY`**) **`emit interpret`** (**`docs/ERROR_SYSTEM.md`** **548–561**). Does **not** claim full structured **`::ast.nodes`** / every in-file **`::execute_*`** path yet.
