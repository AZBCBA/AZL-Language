...
**Principle:** Prefer increments that unlock a **contiguous** region of **`azl/runtime/interpreter/azl_interpreter.azl`** on the **semantic spine** hosted by **`tools/azl_semantic_engine/minimal_runtime.py`** (transitional **Python** stepping surface; **intended spec owner** remains that **`.azl`** file — **Gate G2** separates **spec anchor** vs **spine exec carrier** in the `--semantic-owner` probe; [RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md)). Extend **C minimal** only where **`scripts/check_azl_native_gates.sh`** already enforces parity. **Gates F2–F167** (and successors) are **regression anchors**; they do **not** replace **deeper interpreter smokes** that prove the **real file** advances.
