...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, **two** **`emit interpret`** (same **`say x`**) + **`(cache hit)`** on the second pass, plus **third** (**two-line** **`say`**, **`AZL_SPINE_DEPTH_*`**), **fourth** (**three-line** **`say`**, **`AZL_SPINE_TRIPLE_*`**), **fifth** (**four-line** **`say`**, compact **`Q5a`**–**`Q5d`** for **`::ast.nodes`** **255**-byte cap), **sixth** and **seventh** (duplicate **`say 'AZL_S6_ONLY'`** — **second** tok/parse cache pair; verify **≥4** **`(cache hit)`** lines), **eighth** (**`say 'AZL_S8_MARK'`**) (**`docs/ERROR_SYSTEM.md`** **548–562**). Does **not** claim full structured **`::ast.nodes`** / every in-file **`::execute_*`** path yet.
