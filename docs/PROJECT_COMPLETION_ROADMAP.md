...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, asserts **`emit interpret`** completes through tokenize/parse/execute and the in-file **`execute_complete`** listener (**`Execution complete`** + **`Interpretation complete:`** on stdout; **`docs/ERROR_SYSTEM.md`** **548–555**). Does **not** claim full structured **`::ast.nodes`** / every in-file **`::execute_*`** path yet.
