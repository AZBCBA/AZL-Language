...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, **two** **`emit interpret`** (same code) + in-file **tok_cache**/**ast_cache** **`(cache hit)`** on the second pass, plus a **third** **`emit interpret`** with **two-line** embedded **`say`** code (**`AZL_SPINE_DEPTH_*`** markers) (**`docs/ERROR_SYSTEM.md`** **548–558**). Does **not** claim full structured **`::ast.nodes`** / every in-file **`::execute_*`** path yet.
