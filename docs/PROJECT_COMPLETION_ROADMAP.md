...
| **C — Vertical slice: tokenize** | Match **`listen for "tokenize"`** control/data flow (line **`split`**, **`for ::line_text`**, **`::tokens.concat`**, eol **`push`**, **`::line + 1`**, **`return`**, **`.length`**, **`split_chars`** + **`for ::c`**, **`tz|…`**, double-quoted **`say`**, **`emit … with { tokens: ::tokens }`**, …) | Each feature lands with **fixture + F-gate + ERROR_SYSTEM** rows (**F71**–**F81**; **F77** = outer line loop ~**92–98**; **F78**–**F79** = tail **`say`** + **`emit`** payload **`::var`** bind; **F80** = cache miss + **`tok_misses`**; **F81** = cache hit + **`tok_hits`** + **`::tokens`** from cache + **`return`**); optional **extended smoke** or new **`verify_*`** script proves **more lines** of the real file execute **without silent failure** | Parity gates + smoke / verify script named in PR |
