...
- **P0.1c (release smoke, behavior bridge):** `scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` is step **4**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter, **`AZL_ENTRY=azl.spine.behavior.entry`**, asserts **`emit interpret`** completes through tokenize/parse/execute on **`minimal_runtime`** with documented stubs (**`docs/ERROR_SYSTEM.md`** **548–554**). Does **not** claim full in-file **`::parse_tokens`** / **`::execute_ast`** semantics yet.
