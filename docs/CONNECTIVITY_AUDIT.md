...
| 2 | `scripts/check_azl_native_gates.sh` | **Gate 0:** **`scripts/self_check_release_helpers.sh`** (**`rg`**, **`jq`**, **`bash -n`**, **`azl_release_tag_policy`**, **`release/native/manifest.json`** paths). Then native-only defaults, engine build, C minimal + Python parity (F2–F141), VM tokens, spine resolver + semantic owner G2, tokenizer/brace gate, legacy-host defaults; **executable** `verify_enterprise_native_http_live.sh` |
