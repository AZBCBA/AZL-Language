...
| `release.yml` | Tags `v*.*.*` + **`workflow_dispatch`** | GitHub Release assets (sample bundles + runbooks) via **`gh release create`** in **`scripts/gh_create_sample_release.sh`** (no **`softprops/action-gh-release`** / Node 20 composite). Tag shape: **`vMAJOR.MINOR.PATCH`** plus optional **`-prerelease`** / **`+build`** (see script). **`permissions: contents: write`**. **Dispatch:** required input **`tag`** → **`actions/checkout`** at that ref, **`AZL_RELEASE_TAG`** set for the script. **Tag push:** **`AZL_RELEASE_TAG`** empty, script uses **`GITHUB_REF`**. |
