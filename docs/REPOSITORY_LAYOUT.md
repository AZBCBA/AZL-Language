...
- **Layout:** Subfolders (`benchmarks/`, `state/`, `logs/`, `run/`, `bundles/`, `quarantine/`, …) are defined in **`docs/LOCAL_WORKSPACE_LAYOUT.md`** and created by **`scripts/azl_local_layout.sh`**. Move existing flat files with **`scripts/azl_migrate_local_workspace.sh`**.
...
## 3. `project/` — project workspace (versioned)

**Logs, text captures, JSON, root `.azl` entrypoints, training docs/configs, and mistaken paths** live here — same repo, same product; paths were grouped so the repo root stays clear. See **`project/README.md`**. The **`azl/`** tree remains the canonical language/stdlib implementation.

## 4. Repository root — mixed legacy + ops
