...
**Open next (spine queue):** deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** (bare **`memory|emit`**, longer multi-**`memory|emit`** runs / non-stub memory bodies, real **`execute_listen`**) — **F129+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
