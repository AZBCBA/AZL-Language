...
**Open next (spine queue):** deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** (multi bare **`memory|emit`**, mixed bare + **`with`**, non-stub memory bodies, real **`execute_listen`**) — **F130+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
