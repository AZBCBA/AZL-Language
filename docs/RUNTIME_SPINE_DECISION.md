...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F155** (**say**, **set**, bare **emit**, single- and multi-pair **`emit … with { … }`**, **import**/**link**, **`component ::…`**, braced **`listen for … { say … }`**); next **`listen … then`**, nested blocks, **`memory|…`** rows from parse, larger **`::tokens`** than **Var.v[255]** (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F156+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
