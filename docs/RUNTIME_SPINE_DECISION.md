...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F165** (statement slice incl. **`listen … then`**, inner **`emit`/`emit with`/`set`**, **`component ::…`**, **`memory say …`** through **`memory emit … with { … }`** multi-pair (**F160**–**F164**) and **`listen … { emit … with { … } }`** multi-pair (**F165**); **minimal** **`Var.v`** / **`MAX_VAR_VALUE_LEN`** widened to **512** / **511** bytes so long single-line **`tz|`** buffers fit). Next parity fixtures: multi-statement listen bodies, nested blocks, **`::tokens`** larger than current cap (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F166+**; gap matrix: **[TIER_B_BACKLOG.md](TIER_B_BACKLOG.md)** § **P0.1 — Semantic / runtime gaps**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
