...
**P0c (interpreter-shaped slice, 2026-03-20):** Fixture **`azl/tests/p0_semantic_interpreter_slice.azl`** — `link` + `set ::tokens = []` + `say` (first boot-shaped step toward `::azl.interpreter`). Native gate **F3** asserts **byte-identical stdout** C vs Python on that file. Smoke: **`bash scripts/run_semantic_interpreter_slice.sh`**. Next widening steps copy/pattern more of `azl_interpreter.azl` init into this slice (or extend `minimal_runtime` / C minimal in lockstep) until the real file runs.
