...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F162** (statement slice incl. **`listen … then`**, inner **`emit`/`emit with`/`set`**, **`component ::…`**, **`memory say …`** → **`memory|say|…`** (**F160**), **`memory set …`** → **`memory|set|…`** (**F161**), bare **`memory emit …`** → **`memory|emit|…`** (**F162**)); next parity fixtures for **`memory emit … with`** from tokenized source, multi-pair **`listen|…|emit|…|with|…`** beyond one pair, multi-statement listen bodies, nested blocks, larger **`::tokens`** than **Var.v[255]** (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F163+**; gap matrix: **[TIER_B_BACKLOG.md](TIER_B_BACKLOG.md)** § **P0.1 — Semantic / runtime gaps**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
