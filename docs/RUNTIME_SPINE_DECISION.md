...
**P0b (spine selector + semantic executor phase 1, 2026-03-19):** `AZL_RUNTIME_SPINE` chooses the default `AZL_NATIVE_RUNTIME_CMD` when it is unset — **`scripts/azl_resolve_native_runtime_cmd.sh`**. Values **`c_minimal`** (default) or unset → `scripts/azl_c_interpreter_runtime.sh`; **`azl_interpreter`** or **`semantic`** → `scripts/azl_azl_interpreter_runtime.sh` → **`tools/azl_runtime_spine_host.py`** → **`tools/azl_semantic_engine/minimal_runtime.py`** (Python **host** that **runs** the combined `.azl` sources, including **`azl_interpreter.azl`**, on the supported subset — **not** the final semantic owner; see **Semantic ownership** above). Native gate **F2** diffs stdout vs C on `azl/tests/c_minimal_link_ping.azl`; **`scripts/verify_runtime_spine_contract.sh`** (gate **G**) covers resolver + host env errors + same fixture success. **P0.1a (2026-03-20):** gate **G2** — **`scripts/verify_semantic_spine_owner_contract.sh`** asserts the semantic launcher still **`exec python3`** the spine host, that **`--semantic-owner`** prints **`AZL_SEMANTIC_SPEC_OWNER=azl/runtime/interpreter/azl_interpreter.azl`** then **`AZL_SPINE_EXEC_OWNER=minimal_runtime_python`**, and that the host still imports **`minimal_runtime.run_file`**. **P0.1b (2026-03-21):** **`scripts/verify_azl_interpreter_semantic_spine_smoke.sh`** (step **3** of **`run_full_repo_verification.sh`**) runs the **real** **`azl/runtime/interpreter/azl_interpreter.azl`** through the Python spine host with a harness-only **`::azl.security`** stub (**`azl/tests/stubs/azl_security_for_interpreter_spine.azl`**) so **`init`** completes without **`link`** failure (**`docs/ERROR_SYSTEM.md`** exits **286–290**). **P0.1c (2026-03-22, updated):** **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) — same stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter; **nine** **`emit interpret`** passes (dual same-**`code`** **`say x`** + **`(cache hit)`**, two-line / three-line / four-line embedded **`say`** + **`AZL_SPINE_DEPTH_*`** / **`AZL_SPINE_TRIPLE_*`** / compact **`Q5a`**–**`Q5d`** — **`Var.v`** **255**-byte **`::ast.nodes`** — duplicate **`say 'AZL_S6_ONLY'`** + **second** cache pair; verify **≥4** **`(cache hit)`**, **≥2** lines with **`AZL_S6_ONLY`**, eighth **`say 'AZL_S8_MARK'`**, ninth **`set ::…`** + **`say`** → **`AZL_SPINE_P9_SET_LINE`** — **`::execute_ast`** **`set|…`** on the real chain); asserts **`execute_complete`** / **`Interpretation complete:`**; exits **548–562** and **611** in **`docs/ERROR_SYSTEM.md`**. **Not** a claim of full structured **`::ast.nodes`** / every in-file **`::execute_*`** path. **Full P0** (run `azl_interpreter.azl` as AZL source with full semantics) remains open — see **`docs/PROJECT_COMPLETION_ROADMAP.md`**.
...
**P0execpreilcomponentmemoryemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **`memory|emit|ev|with|k|v`** (queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Memory-section **`emit|…|with|…`** between two **`run_linked_component`** steps — listener stdout before second **`init`**. Gate **F126**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_emit_component_say.azl`** (exits **479–481**, **P0execpreilcomponentmemoryemitcomponentsay**).

**P0execpreilcomponentmemorydualemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **two** **`memory|emit|ev|with|k|v`** rows (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Like **F126**, but two sequential memory-section **`emit|…|with|…`** rows before the second **`component|`** — ordered listener stdout (**`F127_K1`** then **`F127_K2`**) before second **`init`**. Gate **F127**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_dual_emit_component_say.azl`** (exits **482–484**, **P0execpreilcomponentmemorydualemitcomponentsay**).

**P0execpreilcomponentmemorytripleemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **three** **`memory|emit|ev|with|k|v`** rows (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Like **F127**, but three sequential memory-section **`emit|…|with|…`** rows — ordered listener stdout (**`F128_K1`** … **`F128_K3`**) before second **`init`**. Gate **F128**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_triple_emit_component_say.azl`** (exits **485–487**, **P0execpreilcomponentmemorytripleemitcomponentsay**).

**P0execpreilcomponentmemorybareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev`** (no **`|with|…`**, same queue + drain as **F105**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **F126**-shaped interleave but **F105**-style bare memory **`emit`** — listener body does not rely on **`::event.data.*`**. Gate **F129**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_bare_emit_component_say.azl`** (exits **488–490**, **P0execpreilcomponentmemorybareemitcomponentsay**).

**P0execpreilcomponentmemorydualbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **two** bare **`memory|emit|ev`** rows (FIFO, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Like **F129**, but two sequential bare memory **`emit`** rows — ordered listener stdout (**`F130_BARE_1`** then **`F130_BARE_2`**) before second **`init`**. Gate **F130**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_dual_bare_emit_component_say.azl`** (exits **491–493**, **P0execpreilcomponentmemorydualbareemitcomponentsay**).

**P0execpreilcomponentmemorytriplebareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **three** bare **`memory|emit|ev`** rows (FIFO, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Like **F130**, but three sequential bare memory **`emit`** rows — ordered listener stdout (**`F131_BARE_1`** … **`F131_BARE_3`**) before second **`init`**. Gate **F131**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_triple_bare_emit_component_say.azl`** (exits **494–496**, **P0execpreilcomponentmemorytriplebareemitcomponentsay**).

**P0execpreilcomponentmemorymixedbarewithemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev1`** then **`memory|emit|ev2|with|k|v`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Composes **F129** bare drain + **F126**-shaped **`|with|`** payload read (**`::event.data.fk132`**) in one gap between two **`component|`** rows — stdout **`F132_BARE_1`** then payload value **`F132_PAYLOAD`** before second **`init`**. Gate **F132**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_mixed_bare_with_emit_component_say.azl`** (exits **497–499**, **P0execpreilcomponentmemorymixedbarewithemitcomponentsay**).

**P0execpreilcomponentmemorymixedwithbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **`memory|emit|ev1|with|k|v`** then **bare** **`memory|emit|ev2`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Mirror of **F132** — **`|with|`** payload (**`::event.data.fk133`**) runs before **bare** second emit; stdout **`F133_PAYLOAD`** then **`F133_BARE_2`** before second **`init`**. Gate **F133**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_mixed_with_bare_emit_component_say.azl`** (exits **500–502**, **P0execpreilcomponentmemorymixedwithbareemitcomponentsay**).

**P0execpreilcomponentmemorytriplemixedemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **`memory|emit|ev1|with|k1|v1`** then **bare** **`memory|emit|ev2`** then **`memory|emit|ev3|with|k2|v2`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Composes **F132**/**F133** ordering — stdout **`F134_PAY_A`** (**`::event.data.fk134a`**) then **`F134_BARE_MID`** then **`F134_PAY_B`** (**`::event.data.fk134b`**) before second **`init`**. Gate **F134**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_triple_mixed_emit_component_say.azl`** (exits **503–505**, **P0execpreilcomponentmemorytriplemixedemitcomponentsay**).

**P0execpreilcomponentmemorytriplemixedbarewithbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev1`** then **`memory|emit|ev2|with|k|v`** then **bare** **`memory|emit|ev3`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Mirror of **F134** with **bare–with–bare** — stdout **`F135_BARE_1`** then **`F135_PAY_MID`** (**`::event.data.fk135`**) then **`F135_BARE_3`** before second **`init`**. Gate **F135**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_triple_mixed_bare_with_bare_emit_component_say.azl`** (exits **506–508**, **P0execpreilcomponentmemorytriplemixedbarewithbareemitcomponentsay**).

**P0execpreilcomponentmemoryquadmixedbarewithwithbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev1`** then **two** **`memory|emit|ev|with|k|v`** rows (distinct payload keys) then **bare** **`memory|emit|ev4`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Four-row gap — stdout **`F136_BARE_1`** then **`F136_PAY_A`** (**`::event.data.fk136a`**) then **`F136_PAY_B`** (**`::event.data.fk136b`**) then **`F136_BARE_4`** before second **`init`**. Gate **F136**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_quad_mixed_bare_with_with_bare_emit_component_say.azl`** (exits **509–511**, **P0execpreilcomponentmemoryquadmixedbarewithwithbareemitcomponentsay**).

**P0execpreilcomponentmemoryquadmixedwithbarebarewithemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **`memory|emit|ev1|with|k|v`** then **two** bare **`memory|emit|ev`** rows then **`memory|emit|ev4|with|k|v`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Mirror of **F136** — stdout **`F137_PAY_A`** (**`::event.data.fk137a`**) then **`F137_BARE_2`** then **`F137_BARE_3`** then **`F137_PAY_B`** (**`::event.data.fk137b`**) before second **`init`**. Gate **F137**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_quad_mixed_with_bare_bare_with_emit_component_say.azl`** (exits **512–514**, **P0execpreilcomponentmemoryquadmixedwithbarebarewithemitcomponentsay**).

**P0execpreilcomponentmemoryquadmixedbarewithbarewithemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev1`** then **`memory|emit|ev2|with|k|v`** then **bare** **`memory|emit|ev3`** then **`memory|emit|ev4|with|k|v`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Alternating **bare**/**`with`** quad — stdout **`F138_BARE_1`** then **`F138_PAY_A`** (**`::event.data.fk138a`**) then **`F138_BARE_3`** then **`F138_PAY_B`** (**`::event.data.fk138b`**) before second **`init`**. Gate **F138**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_quad_mixed_bare_with_bare_with_emit_component_say.azl`** (exits **515–517**, **P0execpreilcomponentmemoryquadmixedbarewithbarewithemitcomponentsay**).

**P0execpreilcomponentmemoryquadmixedwithwithbarebareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **two** **`memory|emit|ev|with|k|v`** rows (distinct payload keys) then **two** bare **`memory|emit|ev`** rows (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **With**/**with**/**bare**/**bare** quad — stdout **`F139_PAY_A`** (**`::event.data.fk139a`**) then **`F139_PAY_B`** (**`::event.data.fk139b`**) then **`F139_BARE_3`** then **`F139_BARE_4`** before second **`init`**. Gate **F139**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_quad_mixed_with_with_bare_bare_emit_component_say.azl`** (exits **518–520**, **P0execpreilcomponentmemoryquadmixedwithwithbarebareemitcomponentsay**).

**P0execpreilcomponentmemorypentamixedbarewithbarewithbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **bare** **`memory|emit|ev1`** then **`memory|emit|ev2|with|k|v`** then **bare** **`memory|emit|ev3`** then **`memory|emit|ev4|with|k|v`** then **bare** **`memory|emit|ev5`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Penta** **bare**/**`with`** alternation — stdout **`F140_BARE_1`** then **`F140_PAY_A`** (**`::event.data.fk140a`**) then **`F140_BARE_3`** then **`F140_PAY_B`** (**`::event.data.fk140b`**) then **`F140_BARE_5`** before second **`init`**. Gate **F140**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_penta_mixed_bare_with_bare_with_bare_emit_component_say.azl`** (exits **521–523**, **P0execpreilcomponentmemorypentamixedbarewithbarewithbareemitcomponentsay**).

**P0execpreilcomponentmemorypentamixedwithbarewithbarewithemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **`memory|emit|ev1|with|k|v`** then **bare** **`memory|emit|ev2`** then **`memory|emit|ev3|with|k|v`** then **bare** **`memory|emit|ev4`** then **`memory|emit|ev5|with|k|v`** (FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Complementary **penta** **`with`**/**bare** alternation — stdout **`F141_PAY_A`** (**`::event.data.fk141a`**) then **`F141_BARE_2`** then **`F141_PAY_B`** (**`::event.data.fk141b`**) then **`F141_BARE_4`** then **`F141_PAY_C`** (**`::event.data.fk141c`**) before second **`init`**. Gate **F141**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_penta_mixed_with_bare_with_bare_with_emit_component_say.azl`** (exits **524–526**, **P0execpreilcomponentmemorypentamixedwithbarewithbarewithemitcomponentsay**).

**P0execpreilcomponentmemoryhexamixedbarewithbarewithbarewithemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **six** **`memory|emit|…`** rows (**bare** / **`with`** / **bare** / **`with`** / **bare** / **`with`**, FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Hexa** extension of **F140**-style alternation — stdout **`F142_BARE_1`** then **`F142_PA`** (**`::event.data.a`**) then **`F142_BARE_3`** then **`F142_PB`** (**`::event.data.b`**) then **`F142_BARE_5`** then **`F142_PC`** (**`::event.data.c`**) before second **`init`**. Fixture uses short **`import|i`** / event names **`e1`…`e6`** so **`::ast.nodes`** stays within minimal **255-byte** bound. Gate **F142**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_hexa_mixed_bare_with_bare_with_bare_with_emit_component_say.azl`** (exits **527–529**, **P0execpreilcomponentmemoryhexamixedbarewithbarewithbarewithemitcomponentsay**).

**P0execpreilcomponentmemoryhexamixedwithbarewithbarewithbareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **six** **`memory|emit|…`** rows (**`with`** / **bare** / **`with`** / **bare** / **`with`** / **bare**, FIFO queue + drain) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** Complementary **hexa** to **F142** (mirror of **F141** vs **F140**) — stdout **`F143_PA`** (**`::event.data.a`**) then **`F143_BARE_2`** then **`F143_PB`** (**`::event.data.b`**) then **`F143_BARE_4`** then **`F143_PC`** (**`::event.data.c`**) then **`F143_BARE_6`** before second **`init`**. Same compact **`import|i`** / **`e1`…`e6`** encoding. Gate **F143**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_hexa_mixed_with_bare_with_bare_with_bare_emit_component_say.azl`** (exits **530–532**, **P0execpreilcomponentmemoryhexamixedwithbarewithbarewithbareemitcomponentsay**).

**P0execpreilcomponentmemoryheptabareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **seven** bare **`memory|emit|…`** rows (FIFO queue + drain, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Hepta** extension of **F131** — stdout **`F144_BARE_1`** … **`F144_BARE_7`** before second **`init`**. Compact **`import|i`** / **`e1`…`e7`**; **`::ast.nodes`** **186** bytes (under minimal **255**). Gate **F144**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_hepta_bare_emit_component_say.azl`** (exits **533–535**, **P0execpreilcomponentmemoryheptabareemitcomponentsay**).

**P0execpreilcomponentmemoryoctabareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **eight** bare **`memory|emit|…`** rows (FIFO queue + drain, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Octa** extension of **F144** — stdout **`F145_BARE_1`** … **`F145_BARE_8`** before second **`init`**. Compact **`import|i`** / **`e1`…`e8`**; **`::ast.nodes`** **200** bytes (under minimal **255**). Gate **F145**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_octa_bare_emit_component_say.azl`** (exits **536–538**, **P0execpreilcomponentmemoryoctabareemitcomponentsay**).

**P0execpreilcomponentmemorynonabareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **nine** bare **`memory|emit|…`** rows (FIFO queue + drain, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Nona** extension of **F145** — stdout **`F146_BARE_1`** … **`F146_BARE_9`** before second **`init`**. Compact **`import|i`** / **`e1`…`e9`**; **`::ast.nodes`** **214** bytes (under minimal **255**). Gate **F146**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_nona_bare_emit_component_say.azl`** (exits **539–541**, **P0execpreilcomponentmemorynonabareemitcomponentsay**).

**P0execpreilcomponentmemorydecabareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **ten** bare **`memory|emit|…`** rows (FIFO queue + drain, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Deca** extension of **F146** — stdout **`F147_BARE_1`** … **`F147_BARE_10`** before second **`init`**. Compact **`import|i`** / **`e1`…`e10`**; **`::ast.nodes`** **229** bytes (under minimal **255**). Gate **F147**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_deca_bare_emit_component_say.azl`** (exits **542–544**, **P0execpreilcomponentmemorydecabareemitcomponentsay**).

**P0execpreilcomponentmemoryundecabareemitcomponentsay (`execute_ast` **`import|/`link|`** preloop then **`component|…`** then **eleven** bare **`memory|emit|…`** rows (FIFO queue + drain, no **`|with|…`**) then **`component|…`** then **`memory|say|…`**, 2026-03-22):** **Undeca** extension of **F147** — stdout **`F148_BARE_1`** … **`F148_BARE_11`** before second **`init`**. Compact **`import|i`** / **`e1`…`e11`**; **`::ast.nodes`** **244** bytes (under minimal **255**). Gate **F148**: **`azl/tests/p0_semantic_execute_ast_preloop_component_memory_undeca_bare_emit_component_say.azl`** (exits **545–547**, **P0execpreilcomponentmemoryundecabareemitcomponentsay**).

**Open next (spine queue):** **`::parse_tokens`** — **F149–F167** (statement slice incl. **`listen … then`**, inner **`emit`/`emit with`/`set`**, **`component ::…`**, **`memory say …`** through **`memory emit … with { … }`** multi-pair (**F160**–**F164**), **`listen … { emit … with { … } }`** multi-pair (**F165**), **multi-statement `listen { … }`** (**F166** — dual **`say`**; **F167** — **`say` + bare `emit`** at **`eol`**); **minimal** **`Var.v`** / **`MAX_VAR_VALUE_LEN`** widened to **512** / **511** bytes so long single-line **`tz|`** buffers fit). Next parity fixtures: multi-statement listen with **`emit … with`**, **`set` + `emit`**, nested blocks, **`::tokens`** larger than current cap (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F168+**; gap matrix: **[TIER_B_BACKLOG.md](TIER_B_BACKLOG.md)** § **P0.1 — Semantic / runtime gaps**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
