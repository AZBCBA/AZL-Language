...
**Open next (spine queue):** deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** (bare **`memory|emit`**, further multi-**`memory|emit`** / non-stub memory bodies, real **`execute_listen`**) — **F128+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
