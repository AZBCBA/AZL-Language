...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F159** (statement slice incl. **`listen … then`**, inner **`emit`/`emit with`/`set`**, **`component ::…`**); next **`memory|…`** rows from parse, multi-pair **`listen|…|emit|…|with|…`** beyond one pair, multi-statement listen bodies, nested blocks, larger **`::tokens`** than **Var.v[255]** (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F160+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
