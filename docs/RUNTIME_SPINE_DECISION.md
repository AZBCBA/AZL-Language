...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F153** (**say**, **set**, bare **emit**, single- and multi-pair **`emit … with { … }`**, **import**/**link**); next **listen**/**component** AST, larger **`::tokens`** than **Var.v[255]** (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F154+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
