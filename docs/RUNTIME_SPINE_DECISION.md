...
**Open next (spine queue):** **`::parse_tokens`** — **F149–F160** (statement slice incl. **`listen … then`**, inner **`emit`/`emit with`/`set`**, **`component ::…`**, **`memory say …`** → **`memory|say|…`**); next parity fixtures for **`memory set`** / **`memory emit`** (**`with`**) from tokenized source, multi-pair **`listen|…|emit|…|with|…`** beyond one pair, multi-statement listen bodies, nested blocks, larger **`::tokens`** than **Var.v[255]** (or streaming). Deeper **`execute_ast`** vs **`execute_component` / `execute_listen`** — **F161+**; **[PROJECT_COMPLETION_ROADMAP.md](PROJECT_COMPLETION_ROADMAP.md)** phase **E** depth + phase **F** behavior claim.
