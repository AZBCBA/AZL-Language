...
**Target architecture:** **C orchestrates; AZL interprets** — the native engine keeps orchestration (child, bundle, entry); **`azl/runtime/interpreter/azl_interpreter.azl`** is the **intended semantic specification** (what AZL means at full depth). **`tools/azl_semantic_engine/minimal_runtime.py`** and **`tools/azl_interpreter_minimal.c`** are **temporary** **bootstrap**, **parity**, and **contract** carriers on a **narrow** subset — not the long-term spec of the language.

**Current architecture (transitional):** the **default / canonical** enterprise runtime child still runs **C minimal** on the combined bundle; it does **not** execute **`azl_interpreter.azl`** as the semantic engine. Opt-in **`AZL_RUNTIME_SPINE=azl_interpreter`** or **`semantic`** uses the **semantic launcher** (`scripts/azl_azl_interpreter_runtime.sh` → Python spine host → **`minimal_runtime`**) to step **`.azl`** — still a **carrier**, not the steady-state meaning owner. See [RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md) **Current state vs target state** and **Gate G2**.
...
**Gate G2** (`scripts/verify_semantic_spine_owner_contract.sh`): **`tools/azl_runtime_spine_host.py --semantic-owner`** must print exactly two lines (fixed order): (1) **`AZL_SEMANTIC_SPEC_OWNER=azl/runtime/interpreter/azl_interpreter.azl`** — **intended** semantic specification file (**target:** **C orchestrates; AZL interprets**); (2) **`AZL_SPINE_EXEC_OWNER=minimal_runtime_python`** — **today’s exec carrier on the semantic launcher path only** (Python `minimal_runtime` steps **`.azl`**), **not** “Python defines AZL,” **not** a default-spine flip, **not** a claim that **C minimal** is gone from the default enterprise child. Passing G2 does **not** assert full in-file semantic coverage; see [RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md) **Gate G2**.
