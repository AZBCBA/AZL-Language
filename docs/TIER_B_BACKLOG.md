...
|------|-------|-------------------|-----------|
| 1 | **A** | Native gate suite | `bash scripts/check_azl_native_gates.sh` → **all gates passed** (incl. **F2–F87**, **G**, **G2**, **H**) |
...
| 5 | **E** | **Execute** vertical slice | **F86–F87** parity on payload + **`execute_complete`** + **`AZL_USE_VM`** probe (**Layer 1**); extend with VM / **`::execute_ast`** when minimal supports them |
