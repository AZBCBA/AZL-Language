...
Tier B **P0.1c** release crumb: concatenates **`azl/tests/stubs/azl_security_for_interpreter_spine.azl`** + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + **`azl/runtime/interpreter/azl_interpreter.azl`**, runs **`tools/azl_runtime_spine_host.py`** with **`AZL_ENTRY=azl.spine.behavior.entry`**, asserts exit **0**, no **`component not found: ::azl.security`** on stderr, stdout contains **`Pure AZL Interpreter Initialized`**, **`AZL_SPINE_BEHAVIOR_ENTRY_POST_EMIT`**, substring **`Execution complete`**, at least **four** **`Interpretation complete:`** lines (harness **`emit interpret`** ×4 — first two same **`code`** for cache exercise, third two-line and fourth three-line embedded **`say`** chains), at least **two** **`(cache hit)`** substrings (in-file **tokenize** + **parse** on the second interpret), **`AZL_SPINE_DEPTH_A`** / **`AZL_SPINE_DEPTH_B`**, and **`AZL_SPINE_TRIPLE_1`** / **`_2`** / **`_3`**. Prefix **`ERROR[AZL_INTERPRETER_SEMANTIC_SPINE_BEHAVIOR_SMOKE]:`** on stderr.
...
| **556** | Fewer than **four** **`Interpretation complete:`** lines (harness **four** **`emit interpret`**) |
