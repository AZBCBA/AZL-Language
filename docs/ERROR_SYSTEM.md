...
| **639** | Stdout missing **`AZL_SPINE_P17_IF`** (seventeenth interpret **`set ::… = true`** then **`if ( ::… ) { … } otherwise { … }`** — evaluated condition on real behavior path; **`if|`** + host **`execute_ast`**) |
