...
Runs in **`scripts/run_tests.sh`**. Compiles **`tools/azl_core_engine.c`** with **`AZL_CORE_ENGINE_SELFTEST`** (multi-listener dispatch, fixed-depth recursion guard). Prefix **`ERROR[AZL_CORE_ENGINE_VERIFY]:`** on stderr.
...
| **556** | Fewer than **twenty-two** **`Interpretation complete:`** lines (harness **twenty-two** **`emit interpret`**) |
