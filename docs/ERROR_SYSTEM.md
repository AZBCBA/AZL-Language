...
Tier B **P0.1c** release crumb: concatenates **`azl/tests/stubs/azl_security_for_interpreter_spine.azl`** + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + **`azl/runtime/interpreter/azl_interpreter.azl`**, runs **`tools/azl_runtime_spine_host.py`** with **`AZL_ENTRY=azl.spine.behavior.entry`**, asserts exit **0**, no **`component not found: ::azl.security`** on stderr, stdout contains **`Pure AZL Interpreter Initialized`**, **`AZL_SPINE_BEHAVIOR_ENTRY_POST_EMIT`**, substring **`Execution complete`**, at least **seven** **`Interpretation complete:`** lines (harness **`emit interpret`** ×7 — first two same **`code`** for cache exercise, third two-line, fourth three-line, fifth four-line embedded **`say`** chains, sixth and seventh same single-line literal **`say 'AZL_S6_ONLY'`** so the seventh pass hits **tok_cache** + **ast_cache** again), at least **four** **`(cache hit)`** substrings (two pairs: interpret **2** after **`say x`**, interpret **7** after duplicate **`AZL_S6_ONLY`**), **`AZL_SPINE_DEPTH_A`** / **`AZL_SPINE_DEPTH_B`**, **`AZL_SPINE_TRIPLE_1`** / **`_2`** / **`_3`**, fifth-interpret four-line markers **`Q5a`** / **`Q5b`** / **`Q5c`** / **`Q5d`** (compact labels so **`::ast.nodes`** stays within the semantic spine **`Var.v`** **255**-byte cap in **`::parse_tokens`**), and at least **two** stdout lines containing **`AZL_S6_ONLY`** (sixth + seventh interpret). Prefix **`ERROR[AZL_INTERPRETER_SEMANTIC_SPINE_BEHAVIOR_SMOKE]:`** on stderr.
...
| **556** | Fewer than **seven** **`Interpretation complete:`** lines (harness **seven** **`emit interpret`**) |
