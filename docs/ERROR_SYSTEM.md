...
| **556** | Fewer than **eighteen** **`Interpretation complete:`** lines (harness **eighteen** **`emit interpret`**) |
...
| **641** | Stdout contains **`AZL_SPINE_P18_BAD`** (eighteenth interpret then-branch must not run when **`::azl_spine_p18_flag`** is falsey) |
| **642** | Stdout missing **`AZL_SPINE_P18_ELSE`** (eighteenth interpret **`set ::azl_spine_p18_flag = false`** then **`if ( ::azl_spine_p18_flag ) … otherwise …`** — evaluated falsey **`if|`** + host **`execute_ast`**) |
