...
| GitHub Release (sample bundle assets) | Push tag **`v*.*.*`** or **Actions → Release → Run workflow** (input **`tag`**) → **`.github/workflows/release.yml`** → **`scripts/gh_create_sample_release.sh`** (`gh release create`, **`AZL_RELEASE_TAG`** on dispatch; **`permissions: contents: write`**; no Node 20 composite action). See [RELEASE_READY.md](../RELEASE_READY.md) § GitHub Release. |
