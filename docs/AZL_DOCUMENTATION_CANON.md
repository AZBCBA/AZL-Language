...
| Real interpreter **interpret** pipeline (eight **`emit interpret`**, **≥4** **`(cache hit)`**, multi-line **`say`**, duplicate **`AZL_S6_ONLY`**, **`AZL_S8_MARK`**) | **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) + harness **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** — P0.1c; exits **548–562** in [ERROR_SYSTEM.md](ERROR_SYSTEM.md) |
