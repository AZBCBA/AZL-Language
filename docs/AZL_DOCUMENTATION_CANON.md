...
| Real interpreter **interpret** pipeline (stubbed parse/execute bridge) | **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) + harness **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** — P0.1c; exits **548–554** in [ERROR_SYSTEM.md](ERROR_SYSTEM.md) |
