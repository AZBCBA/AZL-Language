...
| GitHub Release (sample bundle assets) | Push tag **`v*.*.*`** or **Actions → Release → Run workflow** (input **`tag`**) → **`.github/workflows/release.yml`**. **Dispatch:** **`gh_verify_remote_tag.sh`** then checkout → **`gh_assert_checkout_matches_tag.sh`** → **`dist/`** → **`gh_create_sample_release.sh`** (`gh release create`, **`AZL_RELEASE_TAG`**; **`permissions: contents: write`**). Tag shape: **`azl_release_tag_policy.sh`**. [CONNECTIVITY_AUDIT.md](CONNECTIVITY_AUDIT.md) §1.1. [RELEASE_READY.md](../RELEASE_READY.md) § GitHub Release, [ERROR_SYSTEM.md](ERROR_SYSTEM.md) (shell helpers + release checkout assertion). |
...
| Real interpreter **interpret** pipeline (nine **`emit interpret`**, **≥4** **`(cache hit)`**, multi-line **`say`**, duplicate **`AZL_S6_ONLY`**, **`AZL_S8_MARK`**, **`AZL_SPINE_P9_SET_LINE`**) | **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) + harness **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** — P0.1c; exits **548–562**, **611** in [ERROR_SYSTEM.md](ERROR_SYSTEM.md) |
...
1. **Predictable semantics** — [language/AZL_CURRENT_SPECIFICATION.md](language/AZL_CURRENT_SPECIFICATION.md); gates F2, F3, G; semantic parity slices F4–F167 (C minimal vs Python **`minimal_runtime`**, through **`::parse_tokens`** **F167** in **`check_azl_native_gates.sh`**).  
2. **Operational strength** — gates script; **`verify_azl_interpreter_semantic_spine_smoke.sh`** (P0.1b) and **`verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (P0.1c) inside **`run_full_repo_verification.sh`**; `verify_native_runtime_live.sh`; full **`run_full_repo_verification.sh`**; [ERROR_SYSTEM.md](ERROR_SYSTEM.md).  
