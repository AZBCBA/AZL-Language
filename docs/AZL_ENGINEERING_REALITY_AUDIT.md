...
| **Correctness** | **Parity gates F2–F166** (native **`check_azl_native_gates.sh`**) prove **subset** C↔Python behavior on **fixtures only**. **`scripts/verify_azl_interpreter_semantic_spine_smoke.sh`** ( **`make verify`** step **3** ) proves the **real** interpreter file reaches **`init`** on the **Python** spine with a **`::azl.security`** stub. **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) proves **nine** **`emit interpret`** passes on **`minimal_runtime`** (dual **`say x`** + **`(cache hit)`**, third two-line + fourth three-line + fifth four-line embedded **`say`** + **`AZL_SPINE_DEPTH_*`** / **`AZL_SPINE_TRIPLE_*`** / **`Q5a`**–**`Q5d`**, duplicate **`say 'AZL_S6_ONLY'`** + **second** **`(cache hit)`** pair — verify **≥4** **`(cache hit)`** lines, eighth **`AZL_S8_MARK`**, ninth **`set|…`** + **`say`** marker **`AZL_SPINE_P9_SET_LINE`**), **`execute_complete`** (parse slice: **`::parse_tokens`** → **`execute_ast`** lines for **say** / **set** / bare **emit** / **`emit … with`** (**F152**–**F153**) / **import** / **link** / **`component|…`** (**F154**) / **`listen|…|say|…`** (**F155**) / **`listen … then { say … }`** (**F156**) / **`listen|…|emit|…`** (**F157**–**F158**) / **`listen|…|set|…`** (**F159**) / **`memory say …`** through **`memory emit … with`** multi-pair (**F160**–**F164**) and **`listen … { emit … with { … } }`** multi-pair (**F165**) — **F149–F165**); **not** a claim of full in-file **`azl_interpreter.azl`** parse/execute semantics. **No** repo-wide proof that RepertoireField or LHA3 semantics match a written spec. |
...
**Native minimal C (2026-03-22):** **`tools/azl_interpreter_minimal.c`** **`.push({ … })` → `tz|…|…|…|…`** uses a **512-byte** **`snprintf`** scratch buffer, then copies into the caller buffer (bounded by **`Var.v[256]`**), so **`gcc -Wformat-truncation`** is not raised on the row formatter; oversized escaped fields truncate consistently with Python **`minimal_runtime`** **`joined[:255]`**. Gates **F123**–**F148** (preloop + **`component|`** / **`memory|emit|…|with|…`** (incl. dual / triple rows) / **bare** / **dual** / **triple bare** / **mixed** bare + **`with`** / **`with`** + bare / **`with`** + bare + **`with`** / **bare** + **`with`** + **bare** / **bare** + **dual** **`with`** + **bare** **`memory|emit|…`** / **`with`** + **dual** **bare** + **`with`** **`memory|emit|…`** / **bare** + **`with`** + **bare** + **`with`** **`memory|emit|…`** / **dual** **`with`** + **dual** **bare** **`memory|emit|…`** / **penta** **bare**/**`with`**/**bare**/**`with`**/**bare** **`memory|emit|…`** / **penta** **`with`**/**bare**/**`with`**/**bare**/**`with`** **`memory|emit|…`** / **hexa** **bare**/**`with`**/**bare**/**`with`**/**bare**/**`with`** **`memory|emit|…`** / **hexa** **`with`**/**bare**/**`with`**/**bare**/**`with`**/**bare** **`memory|emit|…`** / **hepta** **bare** ×7 **`memory|emit|…`** / **octa** **bare** ×8 **`memory|emit|…`** / **nona** **bare** ×9 **`memory|emit|…`** / **deca** **bare** ×10 **`memory|emit|…`** / **undeca** **bare** ×11 **`memory|emit|…`** / memory interleave — **P0execpreilcomponentmemorysetlistenstack**, **P0execpreiltwocomponentmemorysay**, **P0execpreilthreecomponentmemorysay**, **P0execpreilcomponentmemoryemitcomponentsay**, **P0execpreilcomponentmemorydualemitcomponentsay**, **P0execpreilcomponentmemorytripleemitcomponentsay**, **P0execpreilcomponentmemorybareemitcomponentsay**, **P0execpreilcomponentmemorydualbareemitcomponentsay**, **P0execpreilcomponentmemorytriplebareemitcomponentsay**, **P0execpreilcomponentmemorymixedbarewithemitcomponentsay**, **P0execpreilcomponentmemorymixedwithbareemitcomponentsay**, **P0execpreilcomponentmemorytriplemixedemitcomponentsay**, **P0execpreilcomponentmemorytriplemixedbarewithbareemitcomponentsay**, **P0execpreilcomponentmemoryquadmixedbarewithwithbareemitcomponentsay**, **P0execpreilcomponentmemoryquadmixedwithbarebarewithemitcomponentsay**, **P0execpreilcomponentmemoryquadmixedbarewithbarewithemitcomponentsay**, **P0execpreilcomponentmemoryquadmixedwithwithbarebareemitcomponentsay**, **P0execpreilcomponentmemorypentamixedbarewithbarewithbareemitcomponentsay**, **P0execpreilcomponentmemorypentamixedwithbarewithbarewithemitcomponentsay**, **P0execpreilcomponentmemoryhexamixedbarewithbarewithbarewithemitcomponentsay**, **P0execpreilcomponentmemoryhexamixedwithbarewithbarewithbareemitcomponentsay**, **P0execpreilcomponentmemoryheptabareemitcomponentsay**, **P0execpreilcomponentmemoryoctabareemitcomponentsay**, **P0execpreilcomponentmemorynonabareemitcomponentsay**, **P0execpreilcomponentmemorydecabareemitcomponentsay**, **P0execpreilcomponentmemoryundecabareemitcomponentsay**) are documented in **[RUNTIME_SPINE_DECISION.md](RUNTIME_SPINE_DECISION.md)**. Deeper spine work after **F148** is **F149+** — same doc **Open next**, plus **P0tz**, **P0execpreil**, **P0execpreilmemory**, **P0execpreilsaymemory**, **P0execpreilemitmemory**, **P0execpreilmemorylistenemitwithmulti**, **P0execpreilmemorylistenstack**, **P0execpreilsaymemorylistenstack**, **P0execpreilemitmemorylistenstack**, **P0exectreememorylisten**, **P0exectreememorylistenemitwith**, **P0exectreememorylistenemitwithmulti**, **P0exectreememorylistenstack**.
