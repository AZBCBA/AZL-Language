...
| **Correctness** | **Parity gates F2–F153** (native **`check_azl_native_gates.sh`**) prove **subset** C↔Python behavior on **fixtures only**. **`scripts/verify_azl_interpreter_semantic_spine_smoke.sh`** ( **`make verify`** step **3** ) proves the **real** interpreter file reaches **`init`** on the **Python** spine with a **`::azl.security`** stub. **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) proves **`emit interpret`** runs through tokenize/parse/execute on **`minimal_runtime`** (parse slice: **`::parse_tokens`** → **`execute_ast`** lines for **say** / **set** / bare **emit** / **`emit … with { k: v }`** (single-pair **F152**, multi-pair **F153**) / **import** / **link** — **F149–F153**); **not** a claim of full in-file **`azl_interpreter.azl`** parse/execute semantics. **No** repo-wide proof that RepertoireField or LHA3 semantics match a written spec. |
