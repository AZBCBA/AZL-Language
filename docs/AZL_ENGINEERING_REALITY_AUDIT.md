...
| **Correctness** | **Parity gates F2–F160** (native **`check_azl_native_gates.sh`**) prove **subset** C↔Python behavior on **fixtures only**. **`scripts/verify_azl_interpreter_semantic_spine_smoke.sh`** ( **`make verify`** step **3** ) proves the **real** interpreter file reaches **`init`** on the **Python** spine with a **`::azl.security`** stub. **`scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh`** (step **4**) proves **two** **`emit interpret`** passes, **`execute_complete`**, and in-file **tok_cache**/**ast_cache** **`(cache hit)`** on the second pass on **`minimal_runtime`** (parse slice: **`::parse_tokens`** → **`execute_ast`** lines for **say** / **set** / bare **emit** / **`emit … with`** (**F152**–**F153**) / **import** / **link** / **`component|…`** (**F154**) / **`listen|…|say|…`** (**F155**) / **`listen … then { say … }`** (**F156**) / **`listen|…|emit|…`** (**F157**–**F158**) / **`listen|…|set|…`** (**F159**) / **`memory say …`** → **`memory|say|…`** (**F160**) — **F149–F160**); **not** a claim of full in-file **`azl_interpreter.azl`** parse/execute semantics. **No** repo-wide proof that RepertoireField or LHA3 semantics match a written spec. |
