...
4. `bash scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` — Tier B **P0.1c**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter; **eight** **`emit interpret`** + **≥4** **`(cache hit)`** + multi-line depth **`say`** + duplicate **`AZL_S6_ONLY`** + **`AZL_S8_MARK`** (**548–562**).
...
For a fast, scripted check that still exercises **native gates** (F2–F161, G, G2, H, engine build) and the **live** `GET /api/llm/capabilities` probe:
