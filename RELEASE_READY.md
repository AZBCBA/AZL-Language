...
4. `bash scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` — Tier B **P0.1c**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter; **`emit interpret`** through tokenize/parse/execute/**execute_complete** (**548–555**).
...
For a fast, scripted check that still exercises **native gates** (F2–F148, G, G2, H, engine build) and the **live** `GET /api/llm/capabilities` probe:
