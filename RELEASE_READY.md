...
4. `bash scripts/verify_azl_interpreter_semantic_spine_behavior_smoke.sh` — Tier B **P0.1c**: stub + **`azl/tests/harness/azl_interpreter_semantic_spine_behavior_entry.azl`** + interpreter; **seven** **`emit interpret`** + **≥4** **`(cache hit)`** + multi-line depth **`say`** + duplicate **`AZL_S6_ONLY`** (**548–561**).
