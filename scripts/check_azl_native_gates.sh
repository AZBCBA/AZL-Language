...
echo "[gate] F192: C vs Python — parse_tokens listen { dual empty call } → two listen|…|call|…"
F192EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_listen_dual_call.azl"
F192_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F192EX" boot.entry 2>&1)"
f192_c_rc=$?
if [ "$f192_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_listen_dual_call exited $f192_c_rc: $F192_C_OUT"
  exit 825
fi
if ! printf '%s\n' "$F192_C_OUT" | awk 'NR==1{if($0!="listen|f192|call|f192_a")exit 1} NR==2{if($0!="listen|f192|call|f192_b")exit 1} NR==3{if($0!="P0_SEM_F192_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F192 stdout (3 lines), got: $F192_C_OUT"
  exit 825
fi
F192_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F192EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f192_py_rc=$?
if [ "$f192_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_listen_dual_call exited $f192_py_rc: $F192_PY_OUT"
  exit 826
fi
if [ "$F192_C_OUT" != "$F192_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_listen_dual_call" >&2
  echo "C:  $F192_C_OUT" >&2
  echo "Py: $F192_PY_OUT" >&2
  exit 827
fi

echo "[gate] F193: C vs Python — parse_tokens two top-level empty calls → two call|… rows"
F193EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_dual_top_call.azl"
F193_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F193EX" boot.entry 2>&1)"
f193_c_rc=$?
if [ "$f193_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_dual_top_call exited $f193_c_rc: $F193_C_OUT"
  exit 828
fi
if ! printf '%s\n' "$F193_C_OUT" | awk 'NR==1{if($0!="call|f193_a")exit 1} NR==2{if($0!="call|f193_b")exit 1} NR==3{if($0!="P0_SEM_F193_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F193 stdout (3 lines), got: $F193_C_OUT"
  exit 828
fi
F193_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F193EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f193_py_rc=$?
if [ "$f193_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_dual_top_call exited $f193_py_rc: $F193_PY_OUT"
  exit 829
fi
if [ "$F193_C_OUT" != "$F193_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_dual_top_call" >&2
  echo "C:  $F193_C_OUT" >&2
  echo "Py: $F193_PY_OUT" >&2
  exit 830
fi
...
echo "[gate] F196: C vs Python — parse_tokens listen { dual call with identifier args }"
F196EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_listen_dual_call_ident.azl"
F196_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F196EX" boot.entry 2>&1)"
f196_c_rc=$?
if [ "$f196_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_listen_dual_call_ident exited $f196_c_rc: $F196_C_OUT"
  exit 837
fi
if ! printf '%s\n' "$F196_C_OUT" | awk 'NR==1{if($0!="listen|f196|call|f196_a|::F196_A")exit 1} NR==2{if($0!="listen|f196|call|f196_b|::F196_B")exit 1} NR==3{if($0!="P0_SEM_F196_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F196 stdout (3 lines), got: $F196_C_OUT"
  exit 837
fi
F196_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F196EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f196_py_rc=$?
if [ "$f196_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_listen_dual_call_ident exited $f196_py_rc: $F196_PY_OUT"
  exit 838
fi
if [ "$F196_C_OUT" != "$F196_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_listen_dual_call_ident" >&2
  echo "C:  $F196_C_OUT" >&2
  echo "Py: $F196_PY_OUT" >&2
  exit 839
fi

echo "[gate] F197: C vs Python — parse_tokens two top-level calls with identifier args"
F197EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_dual_top_call_ident.azl"
F197_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F197EX" boot.entry 2>&1)"
f197_c_rc=$?
if [ "$f197_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_dual_top_call_ident exited $f197_c_rc: $F197_C_OUT"
  exit 840
fi
if ! printf '%s\n' "$F197_C_OUT" | awk 'NR==1{if($0!="call|f197_a|::F197_A")exit 1} NR==2{if($0!="call|f197_b|::F197_B")exit 1} NR==3{if($0!="P0_SEM_F197_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F197 stdout (3 lines), got: $F197_C_OUT"
  exit 840
fi
F197_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F197EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f197_py_rc=$?
if [ "$f197_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_dual_top_call_ident exited $f197_py_rc: $F197_PY_OUT"
  exit 841
fi
if [ "$F197_C_OUT" != "$F197_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_dual_top_call_ident" >&2
  echo "C:  $F197_C_OUT" >&2
  echo "Py: $F197_PY_OUT" >&2
  exit 842
fi

echo "[gate] F198: C vs Python — parse_tokens listen { call ( ) then say }"
F198EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_listen_call_then_say.azl"
F198_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F198EX" boot.entry 2>&1)"
f198_c_rc=$?
if [ "$f198_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_listen_call_then_say exited $f198_c_rc: $F198_C_OUT"
  exit 843
fi
if ! printf '%s\n' "$F198_C_OUT" | awk 'NR==1{if($0!="listen|f198|call|f198_a")exit 1} NR==2{if($0!="listen|f198|say|F198_MARK")exit 1} NR==3{if($0!="P0_SEM_F198_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F198 stdout (3 lines), got: $F198_C_OUT"
  exit 843
fi
F198_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F198EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f198_py_rc=$?
if [ "$f198_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_listen_call_then_say exited $f198_py_rc: $F198_PY_OUT"
  exit 844
fi
if [ "$F198_C_OUT" != "$F198_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_listen_call_then_say" >&2
  echo "C:  $F198_C_OUT" >&2
  echo "Py: $F198_PY_OUT" >&2
  exit 845
fi

echo "[gate] F199: C vs Python — parse_tokens top-level call then say"
F199EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_top_call_then_say.azl"
F199_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F199EX" boot.entry 2>&1)"
f199_c_rc=$?
if [ "$f199_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_top_call_then_say exited $f199_c_rc: $F199_C_OUT"
  exit 846
fi
if ! printf '%s\n' "$F199_C_OUT" | awk 'NR==1{if($0!="call|f199_fn")exit 1} NR==2{if($0!="say|F199_MARK")exit 1} NR==3{if($0!="P0_SEM_F199_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F199 stdout (3 lines), got: $F199_C_OUT"
  exit 846
fi
F199_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F199EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f199_py_rc=$?
if [ "$f199_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_top_call_then_say exited $f199_py_rc: $F199_PY_OUT"
  exit 847
fi
if [ "$F199_C_OUT" != "$F199_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_top_call_then_say" >&2
  echo "C:  $F199_C_OUT" >&2
  echo "Py: $F199_PY_OUT" >&2
  exit 848
fi

echo "[gate] F200: C vs Python — parse_tokens top-level say then call"
F200EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_top_say_then_call.azl"
F200_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F200EX" boot.entry 2>&1)"
f200_c_rc=$?
if [ "$f200_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_top_say_then_call exited $f200_c_rc: $F200_C_OUT"
  exit 849
fi
if ! printf '%s\n' "$F200_C_OUT" | awk 'NR==1{if($0!="say|F200_MARK")exit 1} NR==2{if($0!="call|f200_fn")exit 1} NR==3{if($0!="P0_SEM_F200_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F200 stdout (3 lines), got: $F200_C_OUT"
  exit 849
fi
F200_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F200EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f200_py_rc=$?
if [ "$f200_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_top_say_then_call exited $f200_py_rc: $F200_PY_OUT"
  exit 850
fi
if [ "$F200_C_OUT" != "$F200_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_top_say_then_call" >&2
  echo "C:  $F200_C_OUT" >&2
  echo "Py: $F200_PY_OUT" >&2
  exit 851
fi

echo "[gate] F201: C vs Python — parse_tokens listen { say then call }"
F201EX="${ROOT_DIR}/azl/tests/p0_semantic_parse_tokens_listen_say_then_call.azl"
F201_C_OUT="$(env -u AZL_USE_VM "$MINI_BIN" "$F201EX" boot.entry 2>&1)"
f201_c_rc=$?
if [ "$f201_c_rc" != 0 ]; then
  echo "ERROR: azl-interpreter-minimal p0_semantic_parse_tokens_listen_say_then_call exited $f201_c_rc: $F201_C_OUT"
  exit 852
fi
if ! printf '%s\n' "$F201_C_OUT" | awk 'NR==1{if($0!="listen|f201|say|F201_MARK")exit 1} NR==2{if($0!="listen|f201|call|f201_fn")exit 1} NR==3{if($0!="P0_SEM_F201_OK")exit 1} END{if(NR!=3)exit 1}'; then
  echo "ERROR: expected F201 stdout (3 lines), got: $F201_C_OUT"
  exit 852
fi
F201_PY_OUT="$(unset AZL_INTERPRETER_DAEMON; env -u AZL_USE_VM AZL_COMBINED_PATH="$F201EX" AZL_ENTRY='boot.entry' python3 "${ROOT_DIR}/tools/azl_runtime_spine_host.py" 2>&1)"
f201_py_rc=$?
if [ "$f201_py_rc" != 0 ]; then
  echo "ERROR: Python spine host p0_semantic_parse_tokens_listen_say_then_call exited $f201_py_rc: $F201_PY_OUT"
  exit 853
fi
if [ "$F201_C_OUT" != "$F201_PY_OUT" ]; then
  echo "ERROR: C vs Python output mismatch on p0_semantic_parse_tokens_listen_say_then_call" >&2
  echo "C:  $F201_C_OUT" >&2
  echo "Py: $F201_PY_OUT" >&2
  exit 854
fi
